...
Bool_t TDCAlign::Process(Long64_t entry)
{
    ...
    // It can be passed to either TDCAlign::GetEntry() or TBranch::GetEntry()
    ...
    //b_SiliconInfo_TDCChannelBack->GetEntry(entry);
    //b_SiliconInfo_TDCValueBack->GetEntry(entry);
    ...
    //for (size_t i = 0; i < TDCChannelBack.size(); ++i)
    //{
        //if (TDCChannelBack[i] >= channel_start && TDCChannelBack[i] < channel_end)
        //{
            //tdc[TDCChannelBack[i] - channel_start]->Fill(TDCValueBack[i] - tof);
        //}
    //}
